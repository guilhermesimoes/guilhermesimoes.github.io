...
Our project was unnecessarily including the types of dozens of development dependencies! Setting the compiler option [`types`] to an empty array prevented all `node_modules/@types` from being automatically included. As if by magic, the return type of `setTimeout` now finally appeared as `number`.

---

We should observe that while using `types` is the correct solution to avoid NodeJS types in a web project, this compiler option can cause the project's tests to fail _if_ they depend on NodeJS types. In that case, we need a `tsconfig.test.json` file to reference those test-specific types.

In our project, we had to do the following changes:

1. Create a `tsconfig.test.json` with `types: ["jest"]` since that's the test framework we use and we want to include the package `@types/jest`.
2. Avoid adding `"node"` to the `types` array. While it would be somewhat accurate to do this, the issue is that by doing that we'd "pollute" the global typing environment during tests. All the `setTimeout`s would have a return type of `number` during our `build` command, but would have a return type of `NodeJS.Timeout` during our `test` command. So our `build` command would work but our `test` command would fail.
3. Rename `global` to [`globalThis`] in our tests since `global` is a NodeJS concept.
4. Use [Triple-Slash Directives] in tests where `node` types are really necessary (for example, when using `fs` or `path`):
```ts
...
While our project is a web project, we were still using _some_ NodeJS types in order to build the project, mostly in this form:
